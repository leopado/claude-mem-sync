...
var MF=function(m){var p=m.split("-");var mn=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return mn[parseInt(p[1])-1]+" "+p[0]};
...
if(tl&&window.Chart){var x=tl.timeline||[];if(C.tl)C.tl.destroy();C.tl=new Chart(Q("#ctl"),{type:"line",data:{labels:x.map(function(t){return MF(t.month)}),datasets:[{label:"Exports",data:x.map(function(t){return t.exports}),borderColor:"#6366f1",backgroundColor:"rgba(99,102,241,.1)",fill:true,tension:.3},{label:"Imports",data:x.map(function(t){return t.imports}),borderColor:"#22c55e",backgroundColor:"rgba(34,197,94,.1)",fill:true,tension:.3}]},options:{plugins:{legend:{labels:{usePointStyle:true,pointStyle:"circle",padding:16}}},scales:{y:{beginAtZero:true,grid:{color:"rgba(255,255,255,.04)"}},x:{grid:{display:false}}}}})}
...
var hi=r[0],tl=r[1];
if(tl&&window.Chart){var x=tl.timeline||[];if(C.sy)C.sy.destroy();C.sy=new Chart(Q("#csy"),{type:"bar",data:{labels:x.map(function(t){return MF(t.month)}),datasets:[{label:"Exported",data:x.map(function(t){return t.exportedObs||0}),backgroundColor:"rgba(99,102,241,.7)",borderRadius:4},{label:"Imported",data:x.map(function(t){return t.importedObs||0}),backgroundColor:"rgba(34,197,94,.7)",borderRadius:4}]},options:{plugins:{legend:{labels:{usePointStyle:true,pointStyle:"circle",padding:16}}},scales:{y:{beginAtZero:true,stacked:true,grid:{color:"rgba(255,255,255,.04)"}},x:{stacked:true,grid:{display:false}}}}})}
...
function loadProfile(dev,proj){
  ...
  F("/api/profiles/"+encodeURIComponent(dev)+(proj?"?project="+proj:"")).then(function(p){
    ...
    if(window.Chart){
      ...
      C.pfTmp=new Chart(Q("#cpf-tempo"),{type:"line",data:{labels:wk.map(function(w){return MF(w.month)}),datasets:[{label:"Observations",data:wk.map(function(w){return w.count}),borderColor:"#06b6d4",backgroundColor:"rgba(6,182,212,.1)",fill:true,tension:.3}]},options:{plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:"rgba(255,255,255,.04)"}},x:{grid:{display:false}}}}});
